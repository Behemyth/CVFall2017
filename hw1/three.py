...
'''
This program takes in 3 arguments: the name of the source video,
the name of the target video, and the name of the output video.

It uses codec MP4V so the output video MUST but .mp4 in order
for the video to output correctly
'''

# Runs through all of the frames in the video
# Returns image that is the average of all the frames
def FlattenImage(vid) :
	# Holds the sum of all the frames' RGB values at each pixel
	sumImg = np.zeros( (int(vid.get(4)), int(vid.get(3)), 3), dtype=np.float )

	# Loop through all of the frames add to sum
	while(True):
		ret, frame = vid.read()
		if (not ret):
			# Reached end of file
	...
	return sumImg / vid.get(7) # property 7 is the number of frames
...
def CreateMask(energy):
	...
	mask = np.zeros( (energy.shape), dtype=np.float )
	...
	for row in range(energy.shape[0]-1,0,-1):
		...
		# Checking edge cases
		if (index == 0):
			minCost = min( energy[row-1][index], energy[row-1][index+1] )

			if (energy[row-1][index+1] == minCost):
				index = index + 1

		elif (index == energy.shape[1]-1):
			minCost = min( energy[row-1][index], energy[row-1][index-1] )
...
def CreateComposite(source, target, mask, outputName):
	compositeVideo = cv2.VideoWriter(outputName, 
									...
									(int(source.get(3)), int(source.get(4))), True)  # setting size of composite to be same as source
	
	# Need to convert the values in the mask from grayscale to 0-1
	mask = mask / 255.0
	mask = mask[:,:,np.newaxis] # Comment out when reading in mask file
	...
	while(True):
		...
		compositeImage = compositeImage.astype('uint8')  # Need to convert type or else it will not be able to write!
...
if __name__ == "__main__":
	...
	# 3rd property of videos is the width
	# 4th property of videos is the height
	...
	# FOR DEBUGGING
	# mask = cv2.imread('twin_mask_asher12.jpg')
